...
function paretoItems(items, totalQty, threshold) {
  ...
  return { count: out.length, qty: acc, pct: totalQty ? (acc / totalQty) * 100 : 0 };
}
...
async function buildMonthInsights(items, totalQty, year, month, dayVal) {
  ...
  const anomHtml = anom.days.length
    ? `<div class="table-wrap"><table>
       ...
       <div style="font-size:.78rem;color:var(--muted);margin-top:.3rem">المتوسط اليومي للأيام التي فيها حركة: ${Math.round(anom.avg).toLocaleString('en')}</div>`
  ...
  box.innerHTML = `
    ...
    <div class="insight-block">
      ...
      <div class="insight-cards">
        <div class="insight-card"><div class="k">50% من الوحدات</div><div class="v">${p50.count}</div><div class="s">صنف</div></div>
        <div class="insight-card"><div class="k">80% من الوحدات</div><div class="v">${p80.count}</div><div class="s">من أصل ${items.length} صنفاً</div></div>
      </div>
    </div>
    <div class="insight-block">
